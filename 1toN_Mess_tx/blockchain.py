...
class Blockchain:
    ...
    Attributes:
        ...
        :open_messsactions: The list of open messsactions
    ...
    def load_data(self):
        """Initialize blockchain + open transactions + open chipsactions + open messsactions data from a file."""
        try:
    ...
    def save_data(self):
        """Save blockchain + open transactions and open chipsactions and messsactions snapshot to a file."""
        try:
    ...
    def proof_of_work(self):
        """Generate a proof of work for all tyoe of open transactions, the hash of the previous block and a random number (which is guessed until it fits)."""
    ...
    def get_message(self, sender=None):
        """Returns all the messages from the senders"""
        if sender == None:
            if self.public_key == None:
                return None
            participant = self.public_key
    ...
    def add_chipsaction(self, sender, recipient, follow, message, signature, amount=1.0, is_receiving=False):
        """ 
        Arguments:
            :sender: The sender of the coins.
        ...
        """
        chipsaction = Chipsaction(sender, recipient, follow, message, signature, amount)
        if Verification.verify_chipsaction(chipsaction, self.get_balance):
            ...
            if not is_receiving:
                for node in self.__peer_nodes:
                    ...
                    try:
                        response = requests.post(url, json={
                                                 'sender': sender, 'recipient': recipient, 'follow': follow, 'message': message, 'amount': amount, 'signature': signature})
                        if response.status_code == 400 or response.status_code == 500:
    ...
    def add_messsaction(self, sender, follower, message, signature, is_receiving=False):
        """ 
        Arguments:
            :sender: The sender of the coins.
        ...
        if Verification.verify_messsaction(messsaction, sender, follower):
            ...
            if not is_receiving:
                for node in self.__peer_nodes:
                    ...
                    try:
                        response = requests.post(url, json={
                                                 'sender': sender, 'follower': follower, 'message': message, 'signature': signature})
                        if response.status_code == 400 or response.status_code == 500:
    ...
    def mine_block(self):
        """Create a new block and add open transactions and chipsactions and messsactions to it."""
        if self.public_key == None:
            return None
        last_block = self.__chain[-1]
